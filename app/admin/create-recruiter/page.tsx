...
import { Briefcase, Mail, User, Building2, CheckCircle, ArrowLeft, Loader2 } from "lucide-react";
...
export default function CreateRecruiter() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await pb.collection("recruiter").create({  // ✅ Changed from "recruiters"
        user: userRecord.id,
      ...
      if (error.status === 400 && error.data?.data?.email) {
        ...
        setErrors({ submit: "You don't have permission to create recruiters" });
      } else if (error.status === 0) {
        setErrors({ submit: "Network error. Please check your connection." });
      } else {
        setErrors({ submit: error.message || "Failed to create recruiter. Please try again." });
      }
    } finally {
  ...
  if (success) {
    return (
      <div className="max-w-2xl mx-auto text-center py-12">
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Recruiter Created Successfully!</h2>
    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="p-6 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-900">Add New Recruiter</h2>
          <p className="text-gray-600 mt-1">Register external recruiter to view student portfolios</p>
        </div>
