...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Quick Actions */}
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm dark:shadow-gray-900/20 border border-gray-200 dark:border-gray-800 p-6">
      ...
      {/* Recent Users */}
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm dark:shadow-gray-900/20 border border-gray-200 dark:border-gray-800">
        <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
          <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">Recently Created</h3>
        ...
        {recentUsers.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200">
            {recentUsers.map((user) => (
              <div key={user.id} className="p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800/50">
...
function StatCard({ title, value, subtitle, icon: Icon, color, href }: any) {
  ...
  return (
    <Link href={href} className="block group">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm dark:shadow-gray-900/20 border border-gray-200 dark:border-gray-800 p-6 hover:shadow-md hover:border-blue-300 transition-all">
...
function QuickAction({ href, icon: Icon, title, description, color }: any) {
  ...
  return (
    <Link
      href={href}
      className="flex items-center gap-4 p-4 rounded-xl border border-gray-200 dark:border-gray-800 hover:border-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:bg-gray-900/50 transition-all group"
    >
      <div className={`p-3 rounded-lg transition-colors ${colors[color as keyof typeof colors]}`}>
        <Icon size={24} />
      </div>
      <div>
        <p className="font-medium text-gray-900 dark:text-gray-100 group-hover:text-blue-600 transition-colors">{title}</p>
